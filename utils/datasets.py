...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    """
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=True, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, prefix=''):
        self.img_size = img_size
        self.augment = augment
        self.hyp = hyp
        ...
        self.albumentations = Albumentations(prob=0.3) if augment else None
    ...
    def __getitem__(self, index):
        ...
        # is_train_day
        str_t = self.img_files_rgb[index_rgb].split(os.sep)[-1]
        is_train_day = str_t[:5] in ['set00','set01','set02']
        is_train_night = str_t[:5] in ['set03','set04','set05']

        hyp = self.hyp
        mosaic = self.mosaic and random.random() < hyp['mosaic']
        # if mosaic and is_train_day:
        if mosaic:
            # print('Load mosaic...')
            # img, labels = load_mosaic(self, index)
            ...
            # if self.augment:
            #     img_rgb, labels_rgb = random_perspective(img_rgb, labels_rgb,
            #                                      degrees=hyp['degrees'],
            #                                      translate=hyp['translate'],
            #                                      scale=hyp['scale'],
            #                                      shear=hyp['shear'],
            #                                      perspective=hyp['perspective'])
            #     img_ir, labels_ir = random_perspective(img_ir, labels_ir,
            #                                      degrees=hyp['degrees'],
            #                                      translate=hyp['translate'],
            #                                      scale=hyp['scale'],
            #                                      shear=hyp['shear'],
        ...
        # if self.augment and is_train_day:
        # if self.augment and is_train_night:
        if self.augment:
            # print('data augment...')
            # Albumentations
            img_rgb, labels_rgb = self.albumentations(img_rgb, labels_rgb) # , p=0.5)
            ...
            # hist_equalize 
            # 对比度抑制自适应直方图均衡CLAHE（Contrast Limited Adaptive Histogram Equalization）
            img_rgb = hist_equalize(img_rgb)
            img_ir = hist_equalize(img_ir)
