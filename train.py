...
def train(hyp, opt, device, tb_writer=None):
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader(test_path, imgsz, batch_size, gs, opt,  # testloader  batch_size * 2
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            tags = ['train/box_loss', 'train/obj_loss', 'train/cls_loss',  # train loss
                    'metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95', 'metrics/LAMR',
            ...
            # Update best mAP
            fi = fitness(np.array(results).reshape(1, -1))  # weighted combination of [P, R, mAP@.5, mAP@.5-.95, LAMR]
            if i == 0: # 防止减去lamr小于0 
                best_fitness = fi
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    if pretrained:
        ...
        model = Model(opt.cfg, ch=6, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    with torch_distributed_zero_first(rank):
        check_dataset(data_dict)  # check
    ...
    if opt.adam:
        # optimizer = optim.Adam(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999))  # adjust beta1 to momentum
        optimizer = optim.AdamW(pg0, lr=hyp['lr0'], betas=(hyp['momentum'], 0.999), eps=1e-5)
    else:
        optimizer = optim.SGD(pg0, lr=hyp['lr0'], momentum=hyp['momentum'], nesterov=True)
    ...
    dataloader, dataset = create_dataloader_rgb_ir(train_path_rgb, train_path_ir, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=True, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                            world_size=opt.world_size, workers=4, #opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('train: '))
    mlc = np.concatenate(dataset.labels, 0)[:, 0].max()  # max label class
    nb = len(dataloader)  # number of batches
    print(mlc)
    ...
    # Process 0
    if rank in [-1, 0]:
        testloader, testdata = create_dataloader_rgb_ir(test_path_rgb, test_path_ir,imgsz, batch_size, gs, opt,  # testloader imgsz_test
                                       hyp=hyp, cache=opt.cache_images and not opt.notest, rect=True, rank=-1,
                                       world_size=opt.world_size, workers=4, #opt.workers,
                                       pad=0.5, prefix=colorstr('val: '))
    ...
    # nw = min(nw, (epochs - start_epoch) / 2 * nb)  # limit warmup to < 1/2 of training
    maps = np.zeros(nc)  # mAP per class
    results = (0, 0, 0, 0, 0, 0, 0, 0)  # P, R, mAP@.5, mAP@.5-.95, lamr, val_loss(box, obj, cls)
    scheduler.last_epoch = start_epoch - 1  # do not move
    scaler = amp.GradScaler(enabled=cuda)
    compute_loss = ComputeLoss(model)  # init loss class
    logger.info(f'Image sizes {imgsz} train, {imgsz} test\n'
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                # pred = model(imgs)  # forward
                pred, ssimloss = model(imgs_rgb, imgs_ir)  # forward
                loss, loss_items = compute_loss(pred, targets.to(device))  # loss scaled by batch_size
                # loss += ssimloss.mean() # 0613
                ssimloss_sum = ssimloss.sum()
                if rank != -1:
                    loss *= opt.world_size  # gradient averaged between devices in DDP mode
                    ssimloss *= opt.world_size
                if opt.quad:
                    loss *= 4.
                    ssimloss  *= 4.
            ...
            scaler.scale(ssimloss_sum).backward() # 0705
        ...
        if rank in [-1, 0]:
            ...
            # Log
            tags = ['train/box_loss', 'train/obj_loss', 'train/cls_loss',  # train loss
                    'metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.5:0.95',  'metrics/LAMR',# metrics
                    'val/box_loss', 'val/obj_loss', 'val/cls_loss',  # val loss
                    'x/lr0', 'x/lr1', 'x/lr2']  # params
            for x, tag in zip(list(mloss[:-1]) + list(results) + lr, tags):
                if tb_writer:
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ...
                # # Save last, best and delete
                # each_epoch = wdir / f'epoch_{epoch}.pt'
                # torch.save(ckpt, each_epoch)
